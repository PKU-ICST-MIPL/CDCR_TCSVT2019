...
cmd:option('-save_dir','output','extracted feature directory.')
...
local test_image = matio.load(path.join(opt.data_dir, 'test_img_pool5.mat')).test_img
test_image = test_image:float():cuda()
img_fea = torch.zeros(test_image:size(1), 200)
local step = 10
...
matio.save(path.join(opt.save_dir, 'img_fea.mat'), img_fea:float())

local test_video = matio.load(path.join(opt.data_dir, 'test_vid_fea.mat')).test_fea
test_video = test_video:float():cuda()
vid_fea = torch.zeros(test_video:size(1), 200)
local step = 10
...
matio.save(path.join(opt.save_dir, 'vid_fea.mat'), vid_fea:float())

local test_aud = matio.load(path.join(opt.data_dir, 'test_aud_mfcc_step_128.mat')).test_aud
test_aud = test_aud:float():cuda()
aud_fea = torch.zeros(test_aud:size(1), 200)
local step = 10
...
matio.save(path.join(opt.save_dir, 'aud_fea.mat'), aud_fea:float())

local test_td = matio.load(path.join(opt.data_dir, 'test_3d.mat')).test_3d
test_td = test_td:float():cuda()
td_fea = torch.zeros(test_td:size(1), 200)
local step = 10
...
matio.save(path.join(opt.save_dir, 'txt_fea.mat'), txt_fea:float())
